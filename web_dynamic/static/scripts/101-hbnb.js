    ...
    toggleReviewsButton.addEventListener('click', function () {
        if (currentState === 'hidden') {
            fetchReviews().then(reviews => {
                ...
                toggleReviewsButton.textContent = 'Hide';
                toggleReviewsButton.setAttribute('data-state', 'visible')
            });
        } else {
            reviewsContainer.innerHTML = '';
            toggleReviewsButton.textContent = 'Show';
            toggleReviewsButton.setAttribute('data-state', 'hidden')
        }
    });

    function fetchReviews() {
        return fetch('http://0.0.0.0:5001/api/v1/get_review/')
        .then(response => response.json())
